...
def create_tonnage_list(
    ...
    vessels: Optional[Tuple[Vessel, ...]] = None,
) -> TonnageList:
    if vessels is None:
        vessels = tuple()
    return TonnageList(vessels, date)
...
def test_has_length() -> None:
    htl = HistoricalTonnageList((create_tonnage_list(), create_tonnage_list()))

    assert len(htl) == 2


def test_has_multiindex_when_converted_to_data_frame() -> None:
    tonnage_list1 = create_tonnage_list(
        datetime(2020, 6, 9, 12), (create_vessel(imo=1), create_vessel(imo=2))
    )
    tonnage_list2 = create_tonnage_list(
        datetime(2020, 6, 8, 15), (create_vessel(imo=3),)
    )
    htl = HistoricalTonnageList([tonnage_list1, tonnage_list2])
...
def test_contains_vessel_data_when_converted_to_data_frame() -> None:
    v = create_vessel()
    ...
    assert df[Column.OPEN_PREDICTION_ACCURACY].to_list() == [
        v.open_prediction_accuracy
    ]
