...
def test_uses_api_key_from_environment_variables_by_default():
    ...
    with patch.dict(
        "os.environ", {"SIGNAL_OCEAN_API_KEY": key_from_env_var},
        clear=True
    ), patch(SESSION_GET) as get:
        connection._make_get_request(None)

    assert (
        get.call_args.kwargs["headers"]["Api-Key"] == key_from_env_var
    )
...
def test_uses_provided_api_key():
    ...
    assert (
        get.call_args.kwargs["headers"]["Api-Key"] == provided_key
    )
...
def test_targets_production_host_by_default():
    ...
    assert get.call_args.args == (
        "https://api-gateway.signalocean.com/",
    )
...
def test_host_can_be_overridden_via_environment_variable(
    ...
    with patch.dict(
        "os.environ",
        {"SIGNAL_OCEAN_API_HOST": overridden_host},
        clear=True,
    ), patch(SESSION_GET) as get:
        connection._make_get_request(None)
...
def test_makes_get_requests_with_specified_parameters():
    ...
    with patch(SESSION_GET) as get:
        connection._make_get_request(
